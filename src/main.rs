...
fn rocket() -> _ {
    ...
    // Enable TLS if local certs are present (generated by helper script with mkcert)
    // Preferred system-wide location
    let cert_path = "/usr/local/share/eidreader/localhost.pem";
    let key_path = "/usr/local/share/eidreader/localhost-key.pem";
    // User-level fallback location (no sudo needed)
    let home = env::var("HOME").unwrap_or_else(|_| String::from("/var/empty"));
    let user_cert_path = format!("{}/.eidreader/localhost.pem", home);
    let user_key_path = format!("{}/.eidreader/localhost-key.pem", home);

    if Path::new(cert_path).exists() && Path::new(key_path).exists() {
        figment = figment
            .merge(("tls.certs", cert_path))
            .merge(("tls.key", key_path));
    } else if Path::new(&user_cert_path).exists() && Path::new(&user_key_path).exists() {
        figment = figment
            .merge(("tls.certs", user_cert_path))
            .merge(("tls.key", user_key_path));
    }
